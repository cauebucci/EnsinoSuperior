...
class Usuario {
	...
	public static async cookie(req: app.Request, res: app.Response = null, admin: boolean = false): Promise<Usuario> {
		...
		if (!cookieStr || cookieStr.length !== 48) {
			...
			await app.sql.connect(async (sql) => {
				let rows = await sql.query("select id, user, nome, idperfil, token from usuario where id = ?", [id]);
				let row: any;
				...
				usuario = new Usuario();
				usuario.id = id;
				usuario.user = row.user as string;
	...
	public static async efetuarLogin(user: string, senha: string, res: app.Response): Promise<[string, Usuario]> {
		if (!user || !senha)
			return ["Usuário ou senha inválidos", null];

		return await app.sql.connect(async (sql) => {
			user = user.normalize().trim().toLowerCase();

			const usuarios: Usuario[] = await sql.query("select id, nome, idperfil, senha from usuario where user = ? and exclusao is null", [user]);
			let usuario: Usuario;
	...
	private static validar(usuario: Usuario, criacao: boolean): string {
		...
		if (criacao) {
			...
			if (!usuario.user || usuario.user.length > 85 || usuario.user.indexOf(' ') >= 0)
				return "Usuário inválido";
		} else {
			if (isNaN(usuario.id))
				return "Id inválido";
	...
	public static async listar(): Promise<Usuario[]> {
		...
		await app.sql.connect(async (sql) => {
			lista = await sql.query("select u.id, u.user, u.nome, p.nome perfil, date_format(u.criacao, '%d/%m/%Y') criacao from usuario u inner join perfil p on p.id = u.idperfil where u.exclusao is null order by u.user asc") as Usuario[];
		});
	...
	public static async obter(id: number): Promise<Usuario> {
		...
		await app.sql.connect(async (sql) => {
			lista = await sql.query("select id, user, nome, idperfil, date_format(criacao, '%d/%m/%Y') criacao from usuario where id = ?", [id]) as Usuario[];
		});
	...
	public static async criar(usuario: Usuario): Promise<string> {
		...
		await app.sql.connect(async (sql) => {
			try {
				await sql.query("insert into usuario (user, nome, idperfil, senha, criacao) values (?, ?, ?, ?, now())", [usuario.user, usuario.nome, usuario.idperfil, await GeradorHash.criarHash(usuario.senha)]);
			} catch (e) {
				if (e.code) {
					switch (e.code) {
						case "ER_DUP_ENTRY":
							res = `O usuário ${usuario.user} já está em uso`;
							break;
	...
	public static async excluir(id: number): Promise<string> {
		...
		return app.sql.connect(async (sql) => {
			...
			await sql.query("update usuario set user = concat('@', id, ':', user), token = null, exclusao = ? where id = ?", [agora, id]);
